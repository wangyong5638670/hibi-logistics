...
import com.ylink.hibiscus.app.logistics.record.RecordPlanAppService;
...
@RestController()
@Slf4j
@RequestMapping("/v1/logistics/record/plan")
public class RecordPlanAppServiceImpl extends BaseAppServiceImpl<RecordPlan,RecordPlanManager> implements RecordPlanAppService {
    ...
    @RequestBody RestfulRequest<ModelResource<RecordPlan>> request) {
    ...
    @RequestBody RestfulRequest<ModelResource<Collection<RecordPlan>>> request) {
        getManager().create(request.getResouces().getEntity());
    }
    @Override
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<RecordPlan>>> list(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<RecordPlan, RecordPlan>> request) {
        ListPage<RecordPlan> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<RecordPlan>>> listWithTerm(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<RecordPlan, Collection<Term>>> request) {
            ListPage<RecordPlan> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
                return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<RecordPlan>>> listWithSearchCondition(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<RecordPlan, SearchCondition>> request) {
        ListPage<RecordPlan> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
            return RestfulResponse.builder()
            .success()
            .setResouces(ModelResource.builder().setEntity(page).build())
