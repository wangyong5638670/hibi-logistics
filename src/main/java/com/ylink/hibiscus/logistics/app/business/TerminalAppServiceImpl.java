 ...
 */
package com.ylink.hibiscus.logistics.app.business;

import com.ylink.hibiscus.app.logistics.business.TerminalAppService;
...
import com.ylink.hibiscus.logistics.service.business.TerminalManager;
...
@RestController()
@Slf4j
@RequestMapping("/v1/logistics/business/terminal")
public class TerminalAppServiceImpl extends BaseAppServiceImpl<Terminal, TerminalManager> implements TerminalAppService {
    ...
    public void create(@Validated @NotNull(message = "请求参数不能为空")
                       @RequestBody RestfulRequest<ModelResource<Terminal>> request) {
    ...
    public void createWithCollection(@Validated @NotNull(message = "请求参数不能为空")
                                     @RequestBody RestfulRequest<ModelResource<Collection<Terminal>>> request) {
        getManager().create(request.getResouces().getEntity());
    }
    @Override
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Terminal>>> list(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Terminal, Terminal>> request) {
        ListPage<Terminal> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Terminal>>> listWithTerm(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Terminal, Collection<Term>>> request) {
        ListPage<Terminal> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Terminal>>> listWithSearchCondition(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Terminal, SearchCondition>> request) {
        ListPage<Terminal> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
