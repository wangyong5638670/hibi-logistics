...
import com.ylink.hibiscus.app.logistics.business.ParcelAppService;
...
@RestController()
@Slf4j
@RequestMapping("/v1/logistics/business/parcel")
public class ParcelAppServiceImpl extends BaseAppServiceImpl<Parcel, ParcelManager> implements ParcelAppService {
    ...
    public void create(@Validated @NotNull(message = "请求参数不能为空")
                       @RequestBody RestfulRequest<ModelResource<Parcel>> request) {
    ...
    public void createWithCollection(@Validated @NotNull(message = "请求参数不能为空")
                                     @RequestBody RestfulRequest<ModelResource<Collection<Parcel>>> request) {
        getManager().create(request.getResouces().getEntity());
    }
    @Override
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Parcel>>> list(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Parcel, Parcel>> request) {
        ListPage<Parcel> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Parcel>>> listWithTerm(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Parcel, Collection<Term>>> request) {
        ListPage<Parcel> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Parcel>>> listWithSearchCondition(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Parcel, SearchCondition>> request) {
        ListPage<Parcel> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
