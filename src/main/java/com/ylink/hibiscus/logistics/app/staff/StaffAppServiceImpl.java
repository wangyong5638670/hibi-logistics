...
import com.ylink.hibiscus.app.logistics.staff.StaffAppService;
...
@RestController()
@Slf4j
@RequestMapping("/v1/logistics/staff/staff")
public class StaffAppServiceImpl extends BaseAppServiceImpl<Staff, StaffManager> implements StaffAppService {
    ...
    public void create(@Validated @NotNull(message = "请求参数不能为空")
                       @RequestBody RestfulRequest<ModelResource<Staff>> request) {
    ...
    public void createWithCollection(@Validated @NotNull(message = "请求参数不能为空")
                                     @RequestBody RestfulRequest<ModelResource<Collection<Staff>>> request) {
        getManager().create(request.getResouces().getEntity());
    }
    @Override
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Staff>>> list(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Staff, Staff>> request) {
        ListPage<Staff> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Staff>>> listWithTerm(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Staff, Collection<Term>>> request) {
        ListPage<Staff> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<Staff>>> listWithSearchCondition(
            @Validated @NotNull(message = "请求参数不能为空")
            @RequestBody RestfulRequest<PaginationResouce<Staff, SearchCondition>> request) {
        ListPage<Staff> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
        return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
