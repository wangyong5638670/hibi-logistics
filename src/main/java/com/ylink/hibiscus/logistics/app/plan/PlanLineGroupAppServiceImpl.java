...
import com.ylink.hibiscus.app.logistics.plan.PlanLineGroupAppService;
...
import com.ylink.hibiscus.logistics.service.plan.PlanLineGroupManager;
...
@RestController()
@Slf4j
@RequestMapping("/v1/logistics/plan/line/group")
public class PlanLineGroupAppServiceImpl extends BaseAppServiceImpl<PlanLineGroup,PlanLineGroupManager> implements PlanLineGroupAppService {
    ...
    @RequestBody RestfulRequest<ModelResource<PlanLineGroup>> request) {
    ...
    @RequestBody RestfulRequest<ModelResource<Collection<PlanLineGroup>>> request) {
        getManager().create(request.getResouces().getEntity());
    }
    @Override
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<PlanLineGroup>>> list(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<PlanLineGroup, PlanLineGroup>> request) {
        ListPage<PlanLineGroup> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<PlanLineGroup>>> listWithTerm(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<PlanLineGroup, Collection<Term>>> request) {
            ListPage<PlanLineGroup> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
                return RestfulResponse.builder()
                .success()
                .setResouces(ModelResource.builder().setEntity(page).build())
    ...
    @ResponseBody
    public RestfulResponse<ModelResource<ListPage<PlanLineGroup>>> listWithSearchCondition(
        @Validated @NotNull(message = "请求参数不能为空")
        @RequestBody RestfulRequest<PaginationResouce<PlanLineGroup, SearchCondition>> request) {
        ListPage<PlanLineGroup> page  = getManager().list(request.getResouces().getPage(),request.getResouces().getSearch());
            return RestfulResponse.builder()
            .success()
            .setResouces(ModelResource.builder().setEntity(page).build())
